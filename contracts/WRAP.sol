...
contract WRAP is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public  marketingWallet = 0x96158D40CF792Ccb243f2c77cdaca02d2a9210A6;
    address public  liquidityWallet = 0x4a2e61e75f96EecD05425a1Ad16952BA648740ab;
    

    uint256 public swapTokensAtAmount = 100 * (10**18); // 0.1% of total supply
    //Anti-dump    
    uint256 public maxTokenPerWallet = 1000 * (10**18);  // 1% of total supply

    uint256 private marketingBuyFees = 3;
    uint256 private marketingSellFees = 5;

    uint256 private liquidityBuyFee = 1;
    uint256 private liquiditySellFee = 0;
    ...
    function setTradingIsPause(bool value) external onlyOwner {
        tradingIsPause = value;
    }
    
    function setSwapTokensAtAmount(uint256 amount) external onlyOwner {
        require(amount >= totalSupply() / 1000, "SwapTokensAtAmount must be greater than or equal to 0.1% of total supply"); 
        swapTokensAtAmount = amount * (10**18);
    }

    function setMaxTokenPerWallet(uint256 amount) external onlyOwner {
        require(amount >= totalSupply() / 10000, "max token per wallet must be greater than or equal to 0.01% of total supply"); 
    ...
    function updateBuyFees(
       ...
       require(newMarketingFee.add(newLiquidityFee).add(newBurnFee) <= 10, "Max buy fees cannot be more than 10%");
    ...
    require(newMarketingFee.add(newLiquidityFee).add(newBurnFee) <= 10, "Max sell fees cannot be more than 10%");
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (
                ...
                require(currentBalanceRecipient + amount <= maxTokenPerWallet,"WRAP: maximum token per wallet amount exceed");
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        // capture the contract's current BROCK balance.
        // this is so that we can capture exactly the amount of BROCK that the
        // swap creates, and not make the liquidity event include any BROCK that
        // has been manually sent to the contract
        uint256 initialBalance = address(this).balance;

        // swap tokens for BROCK
        swapTokensForBROCK(half, payable(address(this)));
        
        countLiquidityFees -= half;

        // how much BROCK did we just swap into?
    ...
    function swapTokensForBROCK(uint256 tokenAmount, address payable account) private {
        if(tokenAmount <= 0){
            return;
        }
        ...
        // generate the rockswap pair path of token -> wbrock
    ...
    function swapAndSendMarketingBROCK() private {
